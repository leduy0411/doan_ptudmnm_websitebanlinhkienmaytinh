                    ...
                    <div class="conversation-item <?= ($activeConversation && $activeConversation['id'] == $conv['id']) ? 'active' : '' ?> <?= $conv['unread_count'] > 0 ? 'unread' : '' ?>"
                        ...
                        <div class="conv-info">
                            <div class="conv-header">
                                <span
                                    class="name"><?= htmlspecialchars($conv['user']['name'] ?? $conv['user_name'] ?? 'Khách') ?></span>
                                <span class="time"><?= timeAgo($conv['last_message_at'] ?? '') ?></span>
                            </div>
                            <p class="last-message">
                                <?= htmlspecialchars(mb_substr($conv['last_message'] ?? '', 0, 40)) ?>...</p>
                        </div>
            ...
            <?php if ($activeConversation): ?>
                <div class="chat-header">
                    <div class="user-info">
                        ...
                        <div>
                            <span
                                class="name"><?= htmlspecialchars($activeConversation['user']['name'] ?? $activeConversation['user_name'] ?? 'Khách') ?></span>
                            <span
                                class="status <?= ($activeConversation['user']['is_online'] ?? false) ? 'online' : 'offline' ?>">
                ...
                <div class="chat-input">
                    <div class="quick-replies">
                        ...
                        <button class="quick-reply-btn"
                            data-text="Bạn có thể liên hệ hotline 1900xxxx để được hỗ trợ nhanh hơn.">
                            📞 Hotline
                        </button>
                    </div>
            ...
            <?php if ($activeConversation): ?>
                ...
                <div class="panel-body">
                    <div class="user-profile">
                        <img src="<?= BASE_URL . ($activeConversation['user']['avatar'] ?? 'assets/images/default-avatar.svg') ?>"
                            alt="Avatar">
                        <h6><?= htmlspecialchars($activeConversation['user']['name'] ?? $activeConversation['user_name'] ?? 'Khách') ?>
                        </h6>
                        ...
                        <div class="recent-orders">
                            <?php foreach ($userRecentOrders ?? [] as $order): ?>
                                <a href="?page=order-detail&id=<?= $order['id'] ?>" class="order-item">
                                    ...
                                    <span
                                        class="order-status <?= $order['status'] ?>"><?= ORDER_STATUSES[$order['status']] ?></span>
...
<style>
    ...
    .recent-orders .order-status.pending {
        background: #fef3c7;
        color: #d97706;
    }

    .recent-orders .order-status.processing {
        background: #dbeafe;
        color: #2563eb;
    }

    .recent-orders .order-status.shipping {
        background: #ede9fe;
        color: #7c3aed;
    }

    .recent-orders .order-status.delivered {
        background: #d1fae5;
        color: #059669;
    }
            ...
            messageForm.addEventListener('submit', function (e) {
                ...
                fetch('<?= BASE_URL ?>api/employee/chat.php', {
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            // Add message to UI - data.data contains the message object
