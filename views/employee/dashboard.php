                ...
                <div class="card-body p-0">
                    <div class="chat-list">
                        <?php foreach ($recentChats as $chat):
                            $isRead = isset($chat['is_read']) ? $chat['is_read'] : true;
                            $userName = $chat['user_name'] ?? $chat['customer_name'] ?? 'Khách';
                            $userAvatar = isset($chat['user']['avatar']) ? $chat['user']['avatar'] : 'assets/images/default-avatar.svg';
                            $lastMessageAt = $chat['last_message_at'] ?? $chat['updated_at'] ?? '';
                            $lastMessage = $chat['last_message'] ?? $chat['content'] ?? '';
                            $unreadCount = $chat['unread_count'] ?? 0;
                            ?>
                            <a href="?page=chat&id=<?= $chat['id'] ?>" class="chat-item <?= !$isRead ? 'unread' : '' ?>">
                                <img src="<?= BASE_URL . $userAvatar ?>" alt="Avatar" class="avatar"
                                    onerror="this.src='<?= BASE_URL ?>assets/images/default-avatar.svg'">
                                <div class="chat-info">
                                    <div class="chat-header">
                                        <span class="name"><?= htmlspecialchars($userName) ?></span>
                                        <span
                                            class="time"><?= !empty($lastMessageAt) ? timeAgo($lastMessageAt) : '' ?></span>
                                    </div>
                                    <p class="last-message"><?= htmlspecialchars(mb_substr($lastMessage, 0, 50)) ?>...</p>
                                </div>
